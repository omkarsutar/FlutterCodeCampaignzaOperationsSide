...
final brandListViewLogicProvider = Provider.autoDispose
    .family<ProcessedBrandListData, String?>((ref, tapCondition) {
      ...
      const routeIdField = 'brands_primary_route';
      ...
      if (tapCondition == 'listWithTodaysEmptyPOs' ||
        ...
        final poStatusAsync = ref.watch(
          brandsByPOStatusProvider(listState.selectedRouteId),
        );

        return poStatusAsync.when(
          data: (poStatusMap) {
            final rawBrands = tapCondition == 'listWithTodaysEmptyPOs'
                ? (poStatusMap['emptyPOs'] ?? []).cast<ModelBrand>()
                : tapCondition == 'listWithTodaysFilledPOs'
                ? (poStatusMap['filledPOs'] ?? []).cast<ModelBrand>()
                : (poStatusMap['noPOs'] ?? []).cast<ModelBrand>();

            return ProcessedBrandListData(
              filteredBrands: filterAndSort(rawBrands),
              brandCount: rawBrands.length,
            );
          },
          loading: () =>
