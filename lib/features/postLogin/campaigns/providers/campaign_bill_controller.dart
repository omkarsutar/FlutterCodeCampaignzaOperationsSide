...
import '../services/campaign_pdf_service.dart';

class CampaignBillState {
...
class CampaignBillController extends StateNotifier<CampaignBillState> {
  final Ref _ref;

  CampaignBillController(this._ref)
    : super(CampaignBillState(selectedPoIds: {}, splitPreferences: {}));

  /// Initialize default selections if not already done
  void initializeSelections(List<ModelCampaign> orders) {
  ...
  void toggleSelectAll(List<ModelCampaign> orders) {
  ...
  Future<void> generateBillPdf(
    ...
    List<ModelCampaign> selectedOrders,
    dynamic adapter,
  ) async {
    ...
    try {
      ...
      final pdfBytes = await CampaignPdfService.generateBillPdf(
        selectedOrders,
        allItems,
        adapter,
      ...
      final fileName =
          'campaigns_bill_${DateTime.now().millisecondsSinceEpoch}.pdf';
  ...
  Future<void> generateCollectionSheetPdf(
    ...
    try {
      final pdfBytes = await CampaignPdfService.generateCollectionSheetPdf(
        selectedOrders,
        adapter,
      );
...
/// Provider for the Campaign Bill Controller
final campaignBillControllerProvider =
    StateNotifierProvider.autoDispose<
      CampaignBillController,
      CampaignBillState
    >((ref) {
      return CampaignBillController(ref);
    });
...
final billAggregatedItemsProvider =
    FutureProvider.autoDispose<List<AggregatedGroup>>((ref) async {
      final state = ref.watch(campaignBillControllerProvider);
