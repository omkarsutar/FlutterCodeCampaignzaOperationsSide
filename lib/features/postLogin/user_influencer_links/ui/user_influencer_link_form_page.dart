...
/// Custom Form Page for User Influencer Link
/// Filters users to only show those with 'user' role
class UserInfluencerLinkFormPage<T> extends ConsumerStatefulWidget {
  ...
  const UserInfluencerLinkFormPage({
    super.key,
    this.entityId,
  ...
  @override
  ConsumerState<UserInfluencerLinkFormPage<T>> createState() =>
      _UserInfluencerLinkFormPageState<T>();
}

class _UserInfluencerLinkFormPageState<T>
    extends ConsumerState<UserInfluencerLinkFormPage<T>> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Custom: Load users for user_id field
      _loadUsers(controller);
  ...
  Future<void> _loadUsers(GenericFormController controller) async {
    try {
      ...
      dynamic usersRes;
      if (roleRes != null) {
        final userRoleId = roleRes['role_id'];
        usersRes = await client
            .from('users')
      ...
      // Fallback: If role wasn't found or returned no users, fetch all users
      if (usersRes == null || (usersRes as List).isEmpty) {
        debugPrint(
          'UserInfluencerLinkFormPage: Role-based user fetch empty or not found, falling back to all users',
        );
        usersRes = await client
            .from('users')
            .select('user_id, full_name')
      ...
      setState(() {
        _localDropdownOptions['user_id'] = userOptions;
      });
    } catch (e) {
      debugPrint('Error loading users: $e');
      SnackbarUtils.showError('Failed to load users: $e');
    }
  }
